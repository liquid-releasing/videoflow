...
class TestComputeSidecar(unittest.TestCase):
    ...
    def test_full_sidecar_shape(self):
        ...
        # hop_ms is the ROUNDED frame spacing and stays an int -- the Tauri
        # layer reads it as u32. The EXACT spacing rides alongside it as
        # integer microseconds, and duration_ms is computed from that, so it
        # no longer compresses by 0.948% (34s over an hour).
        # See tests/test_sidecar_hop_drift.py.
        self.assertEqual(data["hop_ms"], 23)
        self.assertIsInstance(data["hop_ms"], int)
        self.assertEqual(data["hop_us"], 23220)
        self.assertEqual(
            data["duration_ms"],
            round(data["n_frames"] * data["hop_us"] / 1000.0))
...
class TestConstants(unittest.TestCase):
    ...
    def test_defaults_present_and_sensible(self):
        ...
        # 1.2 adds the exact `hop_us`. 1.0 carries a -0.948% compression;
        # 1.1 stored a float hop_ms that the Tauri layer could not parse.
        self.assertEqual(SIDECAR_VERSION, "1.2")
